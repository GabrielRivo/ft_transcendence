...
# --- AUTH SERVICE -------------------------------------------------------------
# Nécessite : jwt_secret (depuis common), github/discord credentials, pepper
if secret_exists "secret/app/auth" "jwt_secret"; then
    log_info "Secrets Auth existants. (Conservation)"
else
    log_warn "Génération initiale des secrets Auth..."
    
    # Génération des secrets OAuth (à remplacer par les vraies valeurs en production)
    GITHUB_CLIENT_ID="${GITHUB_CLIENT_ID:-placeholder}"
    GITHUB_CLIENT_SECRET="${GITHUB_CLIENT_SECRET:-$(generate_pwd)}"
    DISCORD_CLIENT_ID="${DISCORD_CLIENT_ID:-placeholder}"
    DISCORD_CLIENT_SECRET="${DISCORD_CLIENT_SECRET:-$(generate_pwd)}"
    PEPPER="${PEPPER:-$(openssl rand -base64 32)}"
    
    kubectl exec vault-0 -- sh -c "VAULT_TOKEN=${VAULT_ROOT_TOKEN} vault kv put secret/app/auth \
        github_client_id='$GITHUB_CLIENT_ID' \
        github_client_secret='$GITHUB_CLIENT_SECRET' \
        discord_client_id='$DISCORD_CLIENT_ID' \
        discord_client_secret='$DISCORD_CLIENT_SECRET' \
        pepper='$PEPPER'" > /dev/null
        
    log_success "Secrets Auth injectés."
fi

# --- MATCHMAKING SERVICE ------------------------------------------------------
# Nécessite : Configuration du service (ports, paths, etc.)
if secret_exists "secret/app/matchmaking" "service_name"; then
    log_info "Secrets Matchmaking existants. (Conservation)"
else
    log_warn "Génération initiale des secrets Matchmaking..."
    
    # Configuration par défaut pour le matchmaking
    MATCHMAKING_TICK_RATE_MS="${MATCHMAKING_TICK_RATE_MS:-2000}"
    INITIAL_ELO_RANGE="${INITIAL_ELO_RANGE:-50}"
    EXPANSION_STEP_ELO="${EXPANSION_STEP_ELO:-100}"
    EXPANSION_INTERVAL_SEC="${EXPANSION_INTERVAL_SEC:-5}"
    
    kubectl exec vault-0 -- sh -c "VAULT_TOKEN=${VAULT_ROOT_TOKEN} vault kv put secret/app/matchmaking \
        service_name='matchmaking' \
        api_port='3000' \
        db_path='/data/matchmaking.db' \
        matchmaking_tick_rate_ms='$MATCHMAKING_TICK_RATE_MS' \
        initial_elo_range='$INITIAL_ELO_RANGE' \
        expansion_step_elo='$EXPANSION_STEP_ELO' \
        expansion_interval_sec='$EXPANSION_INTERVAL_SEC'" > /dev/null
        
    log_success "Secrets Matchmaking injectés."
fi
